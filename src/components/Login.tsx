...
export const Login: React.FC<LoginProps> = ({ onLoginSuccess }) => {
  ...
  const handleLogin = (e?: React.FormEvent) => {
    ...
    if (!parsedUsername || !parsedPassword) {
      setError('Please fill in all fields (সব ঘর পূরণ করুন)');
    ...
    if (user) {
      ...
      setError('Invalid Username or Password (ভুল ইউজারনেম বা পাসওয়ার্ড)');
      playBeep('select');
    }
  };
      ...
      <div className="w-full max-w-md relative z-10 transition-all duration-300">
        ...
        <div className="bg-slate-900/90 border-2 border-slate-800/80 rounded-2xl p-6.5 shadow-2xl backdrop-blur-md">
          <h2 className="text-lg font-bold text-slate-100 flex items-center gap-2 mb-5 pb-3 border-b border-slate-800">
            <span>🔒</span> Sign In Required (লগইন করুন)
          </h2>
